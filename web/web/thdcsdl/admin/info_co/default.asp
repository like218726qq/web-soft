...
pageno=trim(replace(request("page"),"'",""))
keyword=trim(replace(request("keyword"),"'",""))
if not isnumeric(id_lm) then 
	id_lm=""
end if
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="border">
    <tr class="topbg">
        <td  colspan="2"  align="center">信息中心管理----信息管理首页</td>
    </tr>
    <tr class="tdbg">
        ...
        <TABLE class=border cellSpacing=1 cellPadding=2 width="100%" border=0>
            <TBODY>
              <TR class=title >
                ...
                <TD align=center><STRONG>信息状态</STRONG></TD>
                ...
                if id_lm<>"" then
                    ...
                    sql="select id_lm,xia from info_lm where id_lm="&id_lm&""
                    ...
                    rs.close
                    set rs=nothing
                    if tt<>"" then tt=" and ("&tt&") "
                
                    sub xialm(fid,tt)
                    set rsb=server.CreateObject("adodb.recordset")
                    sql="select id_lm,xia,add_xx from info_lm  where fid="&fid&" order by px desc,id_lm desc"
                    rsb.open sql,conn,1,1
                    while not rsb.eof
                ...
                '关键字搜索
                if keyword<>"" then
                    tt=tt&" and a.title like '%"&keyword&"%'"
                end if
                
                set rs=server.CreateObject("adodb.recordset")
                sql="select a.id,a.lm,a.title,a.color_font,a.img_sl,a.wtime,a.tuijian,a.hot,a.pass,a.read_num,a.px,b.title_lm from info_co a,info_lm b where a.lm=b.id_lm "&tt&" order by a.px desc,a.id desc"
                rs.open sql,conn,1,1
                if not rs.eof then
                    pagenum=25
                    rs.pagesize=pagenum
            ...
            <TR class=tdbg onMouseOver="DoHL();" onMouseOut="DoLL();" onClick="DoSL();">
                ...
                <TD>
                <strong>[</strong><A href="default.asp?id_lm=<%=rs("lm")%>"><%=rs("title_lm")%></A><strong>]</strong><a href="edit.asp?id=<%=rs("id")%>"><font color="<%=rs("color_font")%>"><%=rs("title")%></font></a>
				<%
				if rs("img_sl")<>"" then 
				...
				<!--推荐
				<%if rs("tuijian")="yes" then%>
				<A href="make.asp?ac=tuijian1&id=<%=rs("id")%>" target="theblank">取消</A>|
				<%else%>
				<A href="make.asp?ac=tuijian2&id=<%=rs("id")%>" target="theblank">推荐</A>|
				<%end if%>
				-->
				<!--热点
				<%if rs("hot")="yes" then%>
				<A href="make.asp?ac=hot1&id=<%=rs("id")%>" target="theblank">取消</A>|
				<%else%>
				<A href="make.asp?ac=hot2&id=<%=rs("id")%>" target="theblank">热点</A>|
				<%end if%>
				-->
				<!--屏蔽-->
				<%if rs("pass")="yes" then%>
				...
				<%else%>
				<a href="make.asp?ac=ping2&id=<%=rs("id")%>" target="theblank">取消</a>|
				<%end if%>
                
				<!--删除-->
				<A onClick="return confirm('提示:真的要删除此信息吗？');" href="make.asp?ac=del&id=<%=rs("id")%>" target="theblank">删除</A>
				</TD>
             </TR>
		<%a=a+1
	 	rs.movenext
              ...
              <TBODY><TR>
                <TD height=30 >
                 ...
                 &nbsp; 
				  ...
				  <option value="ping1">屏蔽选中信息</option>
				  <option value="ping2">取消屏蔽选中</option>
				  <!--热点
				  <option value="hot1">热点选中信息</option>
				  <option value="hot2">取消热点选中</option>
				  -->
				 <!--推荐
				 <option value="tuijian1">推荐选中信息</option>
				  <option value="tuijian2">取消推荐选中</option>
				  --> 
...
<table width="100%" height="25"  border="0" cellpadding="0" cellspacing="0" style="border:1px solid #2F78C8; ">
  <tr class=tdbg>
	...
	<%
	if pagenum<>"" then 
		key="&id_lm="&id_lm&"&keyword="&server.URLEncode(keyword)&""
		call listpage(key)
	end if
	%>
	</td>
...
<iframe width="0" height="0" name="theblank" frameborder="0" scrolling="no"></iframe>
