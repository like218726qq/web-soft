...
function Check()
{
lm=document.form1.lm;
title=document.form1.title;
uselink=document.form1.uselink;
link_url=document.form1.link_url;
...
if not rs.eof  then
	...
	a=0
	while not rs.eof
		response.write "shu_lm["&a&"]=new Array('"&rs("id_lm")&"','"&rs("uselink")&"','"&rs("info_from")&"','"&rs("f_body")&"','"&rs("z_body")&"','"&rs("img_sl")&"','"&rs("wtime")&"','"&rs("s_pic")&"','"&rs("s_typ")&"','"&rs("s_wid")&"','"&rs("s_hei")&"');"&chr(10)
		if rs("id_lm")=lm then
			dis_uselink=rs("uselink")
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
    ...
    <tr  class="tdbg" id="dis_uselink" <%if dis_uselink="no" then response.write"style='display:none;'" end if%>>
    <td align="right"><strong>连接地址：</strong></td>
    <td>
    <input type="checkbox" name="uselink" id="uselink" value="yes" onClick="checklink()" <%if uselink="yes" then response.write" checked" end if%>>
    <input name="link_url" type="text" id="link_url" size="50" maxlength="150" value="<%=link_url%>" <% if uselink="yes" then response.write"" else response.write" disabled='yes'"end if%>  ></td>
    </tr>
    <tr class="tdbg" id="dis_info_from" <%if dis_info_from="no" or uselink="yes" then response.write"style='display:none;'" end if%>>           
    ...
    <tr class="tdbg"  id="dis_f_body"  <%if dis_f_body="no" or uselink="yes" then response.write"style='display:none;'" end if%>>
        <td align="right" valign="top"><strong><br>信息精要：</strong></td>
        <td ><textarea name="f_body" cols="72" rows="4" id="f_body"><%=f_body%></textarea></td>
    </tr>
    <tr class="tdbg" id="dis_z_body"  <%if dis_z_body="no"  or uselink="yes"  then response.write"style='display:none;'" end if%>>          
    ...
    <tr class="tdbg" id="dis_wtime"  <%if dis_wtime="no"  or uselink="yes"  then response.write"style='display:none;'" end if%>>
...
<p align="center">   
<INPUT type=submit id=b1 name=b1   value=" 修 改 " > &nbsp;       <input name="Cancel" type="button" id="Cancel" value=" 取 消 " onClick="location='<%=request.ServerVariables("HTTP_REFERER")%>'" style="cursor:hand;">  </p><br>
</form>
