...
<form action="make.asp" name="form1" method="post" target="theblank" onsubmit="return checkyes(form1)" >
  <table width="100%"  border="0" cellpadding="2" cellspacing="1" class="border">
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select a.id,a.title,a.px,b.title_lm from tol_co a,tol_lm b where a.lm=b.id_lm "&tt&" order by a.px desc,a.id desc"
	rs.open sql,conn,1,1
	if not rs.eof then
		pagenum=25
		rs.pagesize=pagenum
   ...
   while not rs.eof and a<=pagenum
    ...
    <tr class=tdbg  onMouseOver="DoHL();" onMouseOut="DoLL();" onClick="DoSL();">
      ...
      <td align="center" ><a href="edit.asp?id=<%=rs("id")%>">修改</a><!--|<a href="make.asp?id=<%=rs("id")%>&ac=del" target="theblank" onClick="return confirm('确定要删除该数据吗?')">删除</a>--></td>
    </tr>
	<%
	a=a+1
	rs.movenext
