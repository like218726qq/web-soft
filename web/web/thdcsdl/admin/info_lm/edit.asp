...
<INPUT name="id" type="hidden" value="<%=id%>">
  ...
  <tr class="tdbg"  id="gaoji" style="display:none;">
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td height="30">
          <input name="add_xx" type="radio" id="add_xx" value="yes" <%if add_xx="yes" then response.write" checked" end if%>>
          是
          <input type="radio" name="add_xx" id="add_xx" value="no"  <%if add_xx="no" then response.write" checked" end if%>>
        否 可以添加信息</td>
      </tr>
      <tr>
        <td height="30">
          <input name="xia" type="radio" id="xia" value="yes" <%if xia="yes" then response.write" checked" end if%>>
          是
          <input type="radio" name="xia" id="xia" value="no" <%if xia="no" then response.write" checked" end if%>>
        否 有下级栏目</td>
      </tr>

      <tr>
        <td height="30">
          <input type="radio" name="uselink" id="uselink" value="yes" <%if uselink="yes" then response.write" checked" end if%>>
          是
          <input name="uselink" type="radio" id="uselink" value="no" <%if uselink="no" then response.write" checked" end if%>>
        否 有外部连接</td>
      </tr> 
      <tr>
        <td height="30">
          <input type="radio" name="info_from" id="info_from" value="yes" <%if info_from="yes" then response.write" checked" end if%>>
          是
          <input name="info_from" type="radio" id="info_from" value="no" <%if info_from="no" then response.write" checked" end if%>>
        否 有信息来源</td>
      </tr>
      <tr>
        <td height="30">
          <input type="radio" name="f_body" id="f_body" value="yes" <%if f_body="yes" then response.write" checked" end if%>>
          是
          <input name="f_body" type="radio" id="f_body" value="no" <%if f_body="no" then response.write" checked" end if%>>
        否 有简要介绍</td>
      </tr>
      <tr>
        <td height="30">
          <input name="z_body" type="radio" id="z_body" value="yes" <%if z_body="yes" then response.write" checked" end if%>>
          是
          <input type="radio" name="z_body" id="z_body" value="no" <%if z_body="no" then response.write" checked" end if%>>
        否 有详细介绍</td>
      </tr>
      <tr>
        <td height="30">
          <input type="radio" name="img_sl" id="img_sl" value="yes" <%if img_sl="yes" then response.write" checked" end if%>  onclick="document.getElementById('tr_s').style.display=''">
          是
          <input name="img_sl" type="radio" id="img_sl" value="no" <%if img_sl="no" then response.write" checked" end if%>  onclick="document.getElementById('tr_s').style.display='none'">
        否 有文件上传</td>
      </tr>
      ...
      <tr>
        <td height="30">
          <input name="wtime" type="radio" id="wtime" value="yes" <%if wtime="yes" then response.write" checked" end if%>>
          是
          <input name="wtime" type="radio" id="wtime" value="no" <%if wtime="no" then response.write" checked" end if%>>
        否 有发布时间</td>
      </tr>
