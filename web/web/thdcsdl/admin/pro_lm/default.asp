...
<br>
<form name="form1" action="make.asp" target="theblank" method="post" onsubmit="return checkyes(form1)">
...
<TABLE class=border cellSpacing=1 cellPadding=2 width="100%" align=center border=0>
  ...
  <TR class=tdbg onMouseOver="DoHL();" onMouseOut="DoLL();" onClick="DoSL();">
	  ...
	  <TD><IMG height=15 src='../images/tree_folder4.gif' width=15 valign='abvmiddle'><A href='edit.asp?id=<%=rsa("id_lm")%>'><STRONG>&nbsp;<%=rsa("title_lm")%>&nbsp;<%=rsa("title_lm_en")%></STRONG></A>&nbsp;<%=rsa("id_lm")%>&nbsp;<%=zt%></TD>
	  <TD align='center'><input name="<%=rsa("id_lm")%>" id="<%=rsa("id_lm")%>" type="text" value="<%=rsa("px")%>" size="5" maxlength="10" disabled></TD>
      <TD align='center'><A href='edit.asp?id=<%=rsa("id_lm")%>'>修改设置</A> | <A href='make.asp?id=<%=rsa("id_lm")%>&ac=del'  onClick="return confirm('提示:真的要删除该分类吗?\r\n\r\n属于该分类的所有信息将被删除')" target="theblank" >删除</A></TD>
  </TR>
  <%if rsa("xia")="yes" then 
  ...
  %>
	...
	response.Write"<a href='edit.asp?id="&rsb("id_lm")&"'>"&rsb("title_lm")&"&nbsp;"&rsb("title_lm_en")&"</a>&nbsp;"&rsb("id_lm")&"&nbsp;"&tt&"</TD>"&chr(10)
	...
	response.write"<TD align='center'> <A href='edit.asp?id="&rsb("id_lm")&"'>修改设置</A> | <A href='make.asp?id="&rsb("id_lm")&"&ac=del' onClick=""return confirm('提示:真的要删除该分类吗?\r\n\r\n属于该分类的所有信息将被删除')""  target='theblank'>删除</A></TD>"&chr(10)
...
<iframe width="0" height="0" scrolling="no" frameborder="0" name="theblank"></iframe>
<br>
<B>说明</B>：<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;1.显示顺序从大到小排列,删除主分类时子分类也相应删除,属于这些分类信息也相应删除<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 2.<font color='red' style='font-weight:bold;'>×</font> 不能添加信息<font color='blue' style='font-weight:bold;'>√</font> 可以添加信息
</body>
</html>
</BODY></HTML>