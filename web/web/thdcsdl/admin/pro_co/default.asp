  ...
  <TBODY>
    <TR>
      <FORM name=form1   action="make.asp" method=post target=theblank   onsubmit="return checkyes(form1)">
        <TD><TABLE class=border cellSpacing=1 cellPadding=2 width="100%" border=0>
            <TBODY>
              <TR class=title >
                ...
                set rs=server.CreateObject("adodb.recordset")
                sql="select a.id,a.lm,a.title,a.img_sl,a.tuijian,a.hot,a.pass,a.read_num,a.px,b.title_lm from pro_co a,pro_lm b where a.lm=b.id_lm "&tt&" order by a.px desc,a.id desc"
                rs.open sql,conn,1,1
                if not rs.eof then
                    pagenum=25
                    rs.pagesize=pagenum
            ...
            <TR class=tdbg onMouseOver="DoHL();" onMouseOut="DoLL();" onClick="DoSL();">
				...
				<!--删除-->
				<A onClick="return confirm('提示:真的要删除此产品吗？');" href="make.asp?ac=del&id=<%=rs("id")%>" target="theblank">删除</A>
				</TD>
             </TR>
		<%a=a+1
	 	rs.movenext
