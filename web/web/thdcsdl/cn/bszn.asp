	...
	did=request.QueryString("did")
	                ...
	                if fid<>"" then
	                  set rs1=server.CreateObject("adodb.recordset")
						...
						sql="select id_lm, title_lm from info_lm where id_lm="&fid&"  order by px desc,id_lm desc"
  ...
  <tr>
	...
	<!--#include file="top1.asp"-->
      ...
      <tr>
		...
		<div style="padding-top:5px;" class="Products">	  首页 >> 办事指南</div>
			  </td></tr>
                ...
                <td valign="top" colspan="3" height="10">
					...
					<%
						   set rs=server.CreateObject("adodb.recordset")
							sql="select * from info_co where pass='yes' and lm=31 order by px desc,id desc"	
						...
						%> 
									 <tr>				
										...
										<a style="text-decoration:none; color:#000000" href="disnews.asp?id=<%=rs("id")%>&did=<%=id_lm%>&dlm=<%=rs("lm")%>" ><font style="color:<%=rs("color_font")%>;"><%=rs("title")%></font></a>
									   </td>
