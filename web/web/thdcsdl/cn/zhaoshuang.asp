              ...
              <%		
				lm=request.QueryString("lm")
				 if lm="" then lm=12
				  ...
				  rs.open "select * from info_co where lm="&lm,conn,1,1
				  if not rs.eof then
				  title=rs("title")					 
				  z_body=rs("z_body")
				    img_sl=rs("img_sl")
			  ...
			  <tr>
				...
				lm=request.QueryString("lm")
				 if lm="" then lm=12
				  ...
				  %>	
				<tr><td>
				<span <%if lm=10 then response.Write("style=""display:none""") end if%>><%=rs("z_body")%></span>	
				<a <%if lm<>10 then response.Write("style=""display:none""") end if%> href="../<%=rs("img_sl")%>"><%=rs("title")%></a>
				</td>
				</tr>
				  <%
				  	  rs.movenext
