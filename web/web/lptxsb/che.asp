...
<%
	...
	if id<>"" and checknum(id) then
		...
		sql="select top 1 * from info_co where pass='yes' and lm=12 order by px desc,id desc"
	end if
...
<table width="1002" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <td height="46" align="center" valign="bottom" style=" background:url(images/neiye-k_11.jpg) left no-repeat "><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="216" height="40" align="center" valign="bottom"><table width="192" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td height="40" align="center" valign="middle" class="STYLE8" style="background:url(images/ny-zuo_12.jpg)">网络测评</td>
              </tr>
            </table></td>
          ...
          <tr>
            ...
            <td width="726" align="left" valign="middle" style="padding-right:30px;"><span style=" float:right">您所在的位置：首页 > 网络测评 > <%=title%></span>
              <span class="STYLE8">网络测评</span></td>
          </tr>
  ...
  <tr>
    <td width="216" align="center" valign="top"><table width="192" border="0" align="center" cellpadding="0" cellspacing="0">
            ...
            <%
	set rs=server.CreateObject("adodb.recordset")
	sql="select id,title from info_co where pass='yes' and lm=12 order by px desc,id desc"
	rs.open sql,conn,1,1
	while not rs.eof
%>
            	<li><a href="che.asp?id=<%=rs("id")%>"<%if id=rs("id") then%> class="bgdw"<%end if%>><%=rs("title")%></a></li>
          <%
    rs.movenext
	wend
	rs.close
