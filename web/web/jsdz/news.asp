...
<%
	...
	if lm<>"" then
		...
		sql="select top 1 id_lm,title_lm from info_lm where fid=1 order by px desc,id_lm asc"
		set rs=server.CreateObject("adodb.recordset")
		rs.open sql,conn,1,1
		if not rs.eof then
			lm=rs("id_lm")
    	...
    	<td valign="top" style="background:url(images/news_06.jpg) bottom repeat-x;border:1px solid #CCCCCC; border-top:none">
   	  ...
   	  <td width="20px"></td>
          ...
          <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-top:0px">
                <tr>
                  <td height="30px" style="background-image:url(images/news1_03.jpg)">当前位置：<a href="index.asp">首页</a> > <a href="news.asp">新闻资讯</a> <%=dao%></td>
                </tr>
          </table>
          <%
                    ...
                    <td width="38%">
                  	...
                  	<td width="62%" style="line-height:20px">
                   	<h2><%=mid(rs("title"),1,18)%></h2>
                        <%=rs("f_body")%><br /><br />
                        <a href="news_show.asp?id=<%=rs("id")%>">&lt;.......更多内容.......&gt;</a>
                  </td>
              </tr>
           </table>
           <%
