...
<%
	...
	if id<>"" and checknum(id) then
		...
		sql="select top 1 * from tol_co where lm=7 order by px desc,id asc"
	end if
...
var files = '<%
	dim temp
	temp=""
	sql="select * from info_co where lm=16 and pass='yes' order by px desc,id asc"
    	...
    	<td valign="top" style="background:url(images/news_06.jpg) bottom repeat-x;border:1px solid #CCCCCC; border-top:none">
   	  ...
   	  <td width="20px"></td>
          ...
          <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-top:0px">
                <tr>
                  <td height="30px" style="background-image:url(images/news1_03.jpg)">Current Location：<a href="index.asp">Home</a> > <a href="yinxiao.asp">Marketing Center</a> > <%=title%></td>
                </tr>
          </table>
