...
<TABLE class=border cellSpacing=1 cellPadding=2 width="100%" border=0>
    <TBODY>
      <TR class=title >
        ...
        set rs=server.CreateObject("adodb.recordset")
        sql="select a.id,a.lm,a.title,a.title_en,a.img_sl,a.tuijian,a.hot,a.pass,a.read_num,a.px,b.title_lm from pro_co a,pro_lm b where a.lm=b.id_lm "&sq&" order by a.px desc,a.id desc"
        rs.open sql,conn,1,1
        if not rs.eof then
            pagenum=25
            rs.pagesize=pagenum
