...
<%
	...
	sql="select title_lm_en from pro_lm where id_lm="&lm&" "
  ...
  <tr>
    ...
    <td width="774" valign="top" bgcolor="#f8f8f8"><table width="774" border="0" cellpadding="0" cellspacing="0" background="images/products_03.jpg">
      <tr>
        ...
        <td width="627" align="right" class="weiz" style="padding-right:20px;">Location：<a href="index.asp">Home</a> &gt;&gt; <a href="new_pro.asp">New Products</a> &gt;&gt; <%=title_en%></td>
      </tr>
    </table>
          ...
          <td style="padding-top:8px; padding-left:11px;">
            <div style="padding:15px; line-height:180%; padding-top:25px;">
                ...
                <div style="line-height:30px; text-align:center;">
					...
					<%
                         set rs=server.CreateObject("adodb.recordset")
                         sql="select id from pro_co where pass='yes' and hot='yes' order by px desc,id desc"
                         ...
                         do while not rs.eof
                            if rs("id")=cint(id) then
                                ...
                                if not rs.bof then
                   %>
                    <a href="new_pro_show.asp?ID=<%=rs("ID")%>"><img src="images/up.gif" border="0" align="absmiddle" /></a>
                    <%
                        end if
                        rs.movenext
                        rs.movenext
                    ...
                    <span style="padding:0px 20px;"><strong style="font-size:14px"><nobr><%=title_en%></nobr></strong></span><span>
                     ...
                     if not rs.eof then  
                    %>
            <a href="new_pro_show.asp?ID=<%=rs("ID")%>"><img src="images/down.gif" border="0" align="absmiddle" /></a>
                    <%
                     end if
                    exit do
                    end if
                ...
                <%=z_body_en%>
                <div class="back" style="text-align:right;"><a style="color:#000000;" href="new_pro.asp">[Return]</a></div>
            </div>
          </td>
        </tr>
      </table>
