...
<%
	...
	sql="select title_lm_en,id_lm from pro_lm where id_lm="&lm&" "
  ...
  <tr>
    ...
    <td width="774" valign="top" bgcolor="#f8f8f8"><table width="774" border="0" cellpadding="0" cellspacing="0" background="images/products_03.jpg">
      <tr>
        ...
        <td width="105" class="zhans">Products</td>
        <td width="627" align="right" class="weiz" style="padding-right:20px;">Location：<a href="index.asp">Home</a> &gt;&gt; <a href="product.asp">Products</a>  <a href="product.asp?lm=<%=id_lm%>"><%=dao%></a> &gt;&gt; <%=title_en%></td>
      </tr>
    </table>
            ...
            <div style="padding:15px; line-height:180%; padding-top:25px;">
                ...
                <div style="line-height:30px; text-align:center;">
                 <span>
					<%
                         set rs=server.CreateObject("adodb.recordset")
                         sql="select id from pro_co where pass='yes' order by px desc,id desc"
                         ...
                         do while not rs.eof
                            if rs("id")=cint(id) then
                                ...
                                if not rs.bof then
                   %>
                    <a href="product_show.asp?ID=<%=rs("ID")%>"><img src="images/up.gif" border="0" align="absmiddle" /></a>
                    <%
                        end if
                        rs.movenext
                        rs.movenext
                    ...
                    <span style="padding:0px 20px;"><strong style="font-size:14px"><nobr><%=title_en%></nobr></strong></span><span>
                        <%
                     if not rs.eof then  
                    %>
            <a href="product_show.asp?ID=<%=rs("ID")%>"><img src="images/down.gif" border="0" align="absmiddle" /></a>
                    <%
                     end if
                    exit do
                    end if
                ...
                <%=z_body_en%>
                <div class="back" style="text-align:right;"><a style="color:#000000;" href="product.asp">[Return]</a></div>
            </div>
          </td>
        </tr>
      </table>
