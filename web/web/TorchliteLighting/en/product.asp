...
<%
	...
	if lm<>"" then
		if not checknum(lm) then
			...
			sql="select id_lm,fid,title_lm_en from pro_lm where id_lm="&lm&""
  ...
  <tr>
    ...
    <td width="774" valign="top" bgcolor="#f8f8f8"><table width="774" border="0" cellpadding="0" cellspacing="0" background="images/products_03.jpg">
      <tr>
        ...
        <td width="627" align="right" class="weiz" style="padding-right:20px;">Location：<a href="index.asp">Home</a> &gt;&gt; <a href="product.asp">Products</a> <%=dao%></td>
      </tr>
    </table>
        ...
        <tr>
          <td style="padding-top:8px; padding-left:11px;"><table border="0" cellspacing="0" cellpadding="0">
            <tr>
              <%	
					set rs=server.CreateObject("adodb.recordset")
					sql="select id,title_en,img_sl from pro_co where pass='yes' "&sq&" order by px desc,id desc"
					rs.open sql,conn,1,1
					if not rs.eof then
						pagenum=12
						rs.pagesize=pagenum
              ...
              <td width="186"><table border="0" cellspacing="0" cellpadding="0">
                ...
                <tr>
                  <td height="30" align="center"><a style="color:#333333;" href="product_show.asp?id=<%=rs("id")%>"><%=getstr(rs("title_en"),24)%></a></td>
                </tr>
                <tr>
                  <td height="15">&nbsp;</td>
                </tr>
