    ...
    <td width="545" valign="top" bgcolor="#f8f8f8"><table width="545" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <%
			...
			sql="select top 1 id,title,img_sl from info_co where lm=3 order by px desc,id desc"
      ...
      <table width="545" border="0" cellspacing="0" cellpadding="0">
        ...
        <%
			set rs=server.CreateObject("adodb.recordset")
			sql="select top 1 id,title,z_body,wtime from info_co where hot='yes' and lm=1 order by px desc,id desc"
			rs.open sql,conn,1,1
			if not rs.eof then
				while not rs.eof
		%>
        <tr>
          ...
          <span class="news_font" style="padding-top:8px; padding-left:8px;"><span class="news_font" style="padding-top:8px; padding-left:8px;"><img src="images/news_tu.jpg" width="84" height="68" class="about_tu" /></span></span><a href="news_show.asp?id=<%=rs("id")%>" title="<%=rs("title")%>"><%=getstr(rs("title"),60)%></a><br />
            <%=left(rs("z_body"),70)%></td>
        </tr>
        <%
				rs.movenext
				wend
        ...
        <tr>
          <td style="padding-left:8px;">
             <%
				set rs=server.CreateObject("adodb.recordset")
				sql="select top 4 id,title,z_body,wtime from info_co where tuijian='yes' and lm=1 order by px desc,id desc"
				rs.open sql,conn,1,1
				if not rs.eof then
					while not rs.eof
			%>
    ...
    <td width="215" valign="top"><table width="215" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="215" border="0" cellpadding="0" cellspacing="0" background="images/zuix_prod.jpg">
          <tr>
            <td width="159" height="32" class="tuij">推荐产品</td>
      ...
      <tr>
        <td height="160" align="center" background="images/zuix_zhong.jpg">
        		<%
set rs=server.createobject("adodb.recordset")
sql="select top 3 * from info_co where pass='yes' and tuijian='yes' and lm=2 order by px desc,id desc"
rs.open sql,conn
dim strhtml
strhtml=""
  do while not rs.eof
...
document.write('<embed src="bcastr.swf" wmode="opaque" FlashVars="bcastr_file='+files+'&bcastr_link='+links+'&bcastr_title='+texts+'&bcastr_config='+swf_config+'"  menu="false" quality="high" width="'+swf_width +'" height="'+ swf_height +'" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />'); document.write('</object>'); 
      ...
      <table width="215" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td height="35" background="images/contact.jpg" class="font_" style="padding-left:40px;">联系我们</td>
        </tr>
        <tr>
