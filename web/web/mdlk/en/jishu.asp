...
<%
	...
	if id<>"" and checknum(id) then
		...
		sql="select top 1 * from info_co where lm=5 order by px desc,id desc"
	end if
...
<%
	set rs=server.CreateObject("adodb.recordset")
	sql="select id,title,color_font,wtime from info_co where pass='yes' "&sq&" and lm=5 order by px desc,id desc"
	rs.open sql,conn,1,1
	if not rs.eof then
		pagenum=15
		rs.pagesize=pagenum
...
%>
	<tr>
		<td align="left" style="margin-top:20px; padding-top:20px;" height="25" width="85%" style="border-bottom:1px dotted #CCCCCC;">
			&nbsp;<img src="images/jj.jpg" width="3" height="5" border="0" />&nbsp;&nbsp;&nbsp;<a href="jishu_show.asp?id=<%=rs("id")%>"><font style="color:<%=rs("color_font")%>;"><%=getstr(rs("title"),40)%></font></a></td>
