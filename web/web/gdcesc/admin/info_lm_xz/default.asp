...
<table width="100%"  border="0" cellpadding="2" cellspacing="1" class="border">
  <tr align="center" class="title">
	...
	sql="select id_lm,title_lm,fid,add_xx,xia,px from info_lm_xz where fid=0 order by px desc,id_lm desc"
	...
	while not rsa.eof
        ...
        <tr class="tdbg"  onMouseOver="DoHL();" onMouseOut="DoLL();" onClick="DoSL();">
            ...
            <TD align='center'><A href='edit.asp?id=<%=rsa("id_lm")%>'>修改设置</A> <%if rsa("fid")<>0 then%>| <A href='make.asp?id=<%=rsa("id_lm")%>&ac=del'  onClick="return confirm('提示:真的要删除该栏目吗?\r\n\r\n属于该栏目的所有信息将被删除')"   >删除</A><%end if%></TD>
        </tr>
		<%
		if rsa("xia")="yes" then 
...
<%
	...
	sub deflm(fid,i)
		...
		sql="select id_lm,title_lm,add_xx,xia,px from info_lm_xz where fid="&fid&" order by px desc,id_lm desc"
