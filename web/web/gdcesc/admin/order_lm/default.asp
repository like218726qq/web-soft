...
<TABLE class=border cellSpacing=1 cellPadding=2 width="100%" align=center border=0>
  ...
  <TR class=tdbg onMouseOver="DoHL();" onMouseOut="DoLL();" onClick="DoSL();">
	  ...
	  <TD><IMG height=15 src='../images/tree_folder4.gif' width=15 valign='abvmiddle'><A href='edit.asp?id=<%=rsa("id_lm")%>'><STRONG><%=rsa("title_lm")%></STRONG></A>&nbsp;<%=rsa("id_lm")%>&nbsp;<%=zt%>
      <%
        if rsa("img_sl")<>"" then 
            response.Write("<a href=../../"&rsa("img_sl")&"  target=_blank><img src=../images/img.gif border=0 onmouseover=popImage(this,'../../"&rsa("img_sl")&"'); onmouseout=hideLayer(); ></a>")
        end if
        %>
      </TD>
  ...
  %>
	...
	sql="select * from order_lm where fid="&fid&" order by px desc,id_lm desc"
