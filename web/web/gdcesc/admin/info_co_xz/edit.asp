...
'获取该信息所在栏目是否可添加信息
set rs=server.CreateObject("adodb.recordset")
sql="select add_xx from info_lm_xz where id_lm="&lm&""
rs.open sql,conn,1,1
if rs.eof then
...
set rs=server.CreateObject("adodb.recordset")
sql="select * from info_lm_xz  order by px desc,id_lm desc"
rs.open sql,conn
if not rs.eof  then
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
    ...
    <tr class="tdbg" >           
     <td width="120" align="right"><strong>信息标题：</strong></td>          
      <td ><input name="title" type="text" id="title"  size="44" maxlength="650" value="<%=title%>">              
