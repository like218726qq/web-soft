...
if id_lm<>"" then
	if not checknum(id_lm) then
		...
		sql="select id_lm,xia from teach_lm where id_lm="&id_lm&""
		...
		sql="select id_lm,xia,add_xx from teach_lm  where fid="&fid&" order by px desc,id_lm desc"
...
<TABLE class=border cellSpacing=1 cellPadding=2 width="100%" border=0>
    <TBODY>
      <TR class=title >
        ...
        <%
        set rs=server.CreateObject("adodb.recordset")
        sql="select a.id,a.lm,a.title,a.color_font,a.img_sl,a.wtime,a.tuijian,a.hot,a.pass,a.read_num,a.px,b.title_lm from teach_co a,teach_lm b where a.lm=b.id_lm "&sq&" order by a.px desc,a.id desc"
        rs.open sql,conn,1,1
        if not rs.eof then
            pagenum=25
            rs.pagesize=pagenum
