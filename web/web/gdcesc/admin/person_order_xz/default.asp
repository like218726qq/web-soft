...
<FORM name="form1"   action="make.asp" method="post" target="theBlank" onsubmit="return checkyes(form1)">
		...
		sql="select a.id,a.order_no,a.order_zt,a.pay_zt,a.send_zt,a.wtime,b.username from person_order_xz a left join person b on a.username=b.username  where 1=1 "&tt&" order by a.id desc"
       ...
       <TR class=tdbg  onMouseOver="DoHL();" onMouseOut="DoLL();" onClick="DoSL();">
                ...
                <TD align=middle><%=rs("username")%>&nbsp;<%=p_dj(rs("person_dj"))%></TD>
				<td align="center"><%=rs("wtime")%></td>
				<TD align=left><%=order_z(rs("order_zt"))%>&nbsp;<%=pay_z(rs("send_zt"))%>&nbsp;<%=send_z(rs("send_zt"))%></TD>
