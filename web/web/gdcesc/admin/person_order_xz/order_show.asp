...
sql="select * from person_order_xz where id = "&id
...
<TBODY>
              <TR class="topbg">
                <TD align=middle><STRONG>图片</STRONG></TD>
                <TD align=middle><STRONG>产品</STRONG></TD>
       ...
       <TR class=tdbg  onMouseOver="DoHL();" onMouseOut="DoLL();" onClick="DoSL();">
            <TD align=middle width=90 ><a href="../../product.asp?id=<%=rsa("pro_id")%>" target="_blank"><img src="../../<%=rsa("pro_img_sl")%>" width="50" height="50" border="0" /></a></TD>
<TD>
            <a href="../../product.asp?id=<%=rsa("pro_id")%>" target="_blank"><%=rsa("pro_title")%></a><br />
            <span>
编号:<%=rsa("pro_no")%><br />
            </span>
            </TD>
...
<TR class=tdbg  >
        ...
        <TD colspan="3"><%=rs("order_no")%></TD>
    </TR>
      <TR class=tdbg  >
        <TD height="20" align=right class="tdbg2"> 帐号： </TD>
        <TD width="250"><%=rs("username")%> &nbsp; <%
		set rsa=server.CreateObject("adodb.recordset")
		sql="select person_dj from person where username='"&rs("username")&"'"
		rsa.open sql,conn,1,1
		if not rsa.eof then
			response.write p_dj(rsa("person_dj"))
		end if
		rsa.close
		set rsa=nothing
		%></TD>
    ...
    <TR class=tdbg  >
        ...
        <TD colspan="3"><%=get_pay_type(rs("pay_type"))%></TD>
    </TR>
    <TR class=tdbg  >
        <TD width="150" height="20" align=right class="tdbg2"> 已付费用： </TD>
        <TD colspan="3"><%if rs("pay_money")>0 then response.write "￥"&formatnum(rs("pay_money")) else response.write"未付款" end if%></TD>
    </TR>
<%
...
%>
 <tr>
      <td colspan="4" class="tdbg3" style="padding-left:75px;"><strong>购买人信息：</strong></td>
      </tr>
      <TR class=tdbg  >
  ...
  <TR class=tdbg  >
        <TD height="20" align=right class="tdbg2"> 积分： </TD>
        <TD colspan="3"><%=jf%>分</TD>
  </TR>
...
%>
  ...
  <TR class=tdbg  >
    ...
    <TR class=tdbg  >
        <TD height="20" align=right class="tdbg2"> 要求： </TD>
...
<input type="radio" name="pay_zt" value="yes" <%if rs("pay_zt")="yes" then response.write " checked=""checked""" end if%>/> 已付款         </td>
       ...
       <tr>
         <td width="150" align="right" bgcolor="#E3EAED" class="tdbg2">发货状态:</td>
         <td colspan="3" class="tdbg">
<input type="radio" name="send_zt" value="no" <%if rs("send_zt")="no" then response.write " checked=""checked""" end if%>/> 未发货
<input type="radio" name="send_zt" value="yes" <%if rs("send_zt")="yes" then response.write " checked=""checked""" end if%>/> 已发货         </td>
       </tr>
