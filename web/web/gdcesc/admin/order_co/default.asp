...
if id_lm<>"" then
	if not checknum(id_lm) then
		...
		sql="select id_lm,xia from order_lm where id_lm="&id_lm&""
		...
		sql="select id_lm,xia,add_xx from order_lm  where fid="&fid&" order by px desc,id_lm desc"
    ...
    <TR class="tdbg3" >
      ...
      <TD><form id="sform" name="sform" method="get" action="default.asp">
           <select name="id_lm">
            ...
            sql="select id_lm,title_lm,add_xx,xia from order_lm where fid=0 order by px desc,id_lm desc"
            ...
            sub addinfo(fid,i)
            i=i&"&nbsp;"
            sql="select id_lm,title_lm,add_xx,xia from order_lm where fid="&fid&" order by px desc,id_lm desc"
            Set rsb= Server.CreateObject("ADODB.Recordset")
            rsb.open sql,conn,1,1
            while not rsb.eof
...
<TABLE class=border cellSpacing=1 cellPadding=2 width="100%" border=0>
    <TBODY>
      <TR class=title >
        ...
        set rs=server.CreateObject("adodb.recordset")
        sql="select a.id,a.lm,a.title,a.img_sl,a.tuijian,a.hot,a.pass,a.read_num,a.px,b.title_lm from order_co a,order_lm b where a.lm=b.id_lm "&sq&" order by a.px desc,a.id desc"
        rs.open sql,conn,1,1
        if not rs.eof then
            pagenum=25
            rs.pagesize=pagenum
