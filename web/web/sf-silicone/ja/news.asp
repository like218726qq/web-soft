...
<%
	...
	if lm<>"" then
	    ...
	    a=1
		sql="select id_lm,title_lm from info_lm where fid=12 order by px desc,id_lm desc"
		set rs=server.CreateObject("adodb.recordset")
		rs.open sql,conn,1,1
		if not rs.eof then
			title_lm="All News"
...
<%
	  ...
	  response.Write("<div style='width:600px; margin:0 auto; padding-top:20px;'>情報更新...</div>")
	end if
	rs.close
	set rs=nothing
%> 
