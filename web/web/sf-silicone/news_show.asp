...
<div style="padding:20px">
    ...
    <div style="text-align:center; line-height:22px; padding-bottom:5px;">Post time：<%=wtime%> | Source：<%=info_from%> | Author：<%=info_author%></div>
        ...
        if not rs.eof then
         ...
         do while not rs.eof
            if rs("id")=cint(id) then
                rs.MovePrevious
                if not rs.bof then
                    response.write"上一篇：<a href=""news_show.asp?id="&rs("id")&""">"&rs("title")&"</a>"
                else
                    response.write "上一篇：已经是第一篇"
                end if
                rs.movenext
                rs.movenext
                response.write "<br/>"
                if not rs.eof then
                    response.write"下一篇：<a href=""news_show.asp?id="&rs("id")&""">"&rs("title")&"</a>"
                else
                    response.write "下一篇：已经是最后一篇"
                end if
                exit do
            end if
         rs.movenext
