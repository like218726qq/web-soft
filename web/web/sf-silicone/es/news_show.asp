...
<table width="1024" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="733" valign="top" bgcolor="#FFFFFF"><table width="733" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td height="38" class="zhans"><A href="news.asp">noticias</A></td>
      </tr>
    </table>
...
<div style="padding:20px">
    ...
    <div style="text-align:center; line-height:22px; padding-bottom:5px;">Publica tiempo：<%=wtime%> | fuente：<%=info_from%> | autor：<%=info_author%></div>
    <div style="line-height:180%; text-align:justify;text-justify:inter-ideograph"><%=z_body%></div>
    <div style="text-align:right;"><a href="javascript:history.back();">[espalda]</a></div>
    <div class="nextlink">
		<%
        set rs=server.CreateObject("adodb.recordset")
        ...
        if not rs.eof then
         ...
         do while not rs.eof
            if rs("id")=cint(id) then
                rs.MovePrevious
                if not rs.bof then
                    response.write"Anterior：<a href=""news_show.asp?id="&rs("id")&""">"&rs("title")&"</a>"
                else
                    response.write "Anterior：Ya la primera"
                end if
                rs.movenext
                rs.movenext
                response.write "<br/>"
                if not rs.eof then
                    response.write"Próximo：<a href=""news_show.asp?id="&rs("id")&""">"&rs("title")&"</a>"
                else
                    response.write "Próximo：Ya pasado un"
                end if
                exit do
            end if
         rs.movenext
