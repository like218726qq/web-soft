...
<TABLE class=border cellSpacing=1 cellPadding=2 width="100%" border=0>
    <TBODY>
      <TR class=title >
        ...
        <%
        set rs=server.CreateObject("adodb.recordset")
        sql="select a.pro_id,a.id,a.lm,a.title,a.color_font,a.img_sl,a.wtime,a.tuijian,a.hot,a.pass,a.read_num,a.px,b.title_lm from img_co a,img_lm b where a.lm=b.id_lm "&sq&" order by a.px desc,a.id desc"
		'response.Write sql 
		'response.End()
        rs.open sql,conn,1,1
        if not rs.eof then
