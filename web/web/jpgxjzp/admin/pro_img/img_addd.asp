...
'如有pro_id等于空，就创建一个临时id保存再session里
if pro_id="" or not checknum(pro_id) then
	Randomize
	if session("pro_id")="" or not checknum(session("pro_id")) then
		sRnd=Int(900*Rnd)+100
		session("pro_id")=hour(now)&minute(now)&second(now)&sRnd
		pr_id=session("pro_id")
	else
		pr_id=session("pro_id")
	end if
else
...
'保存到数据库
openconn
conn.execute("insert into pro_img (pro_id,img_sl)values("&pr_id&",'"&path&"/"&sSaveFileName&"')")
