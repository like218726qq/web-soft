...
<form action="make.asp" name="form1" method="post" onSubmit="return checkyes(form1)" >
  <table width="100%"  border="0" cellpadding="2" cellspacing="1" class="border">
	...
	<%
	set rs=server.CreateObject("adodb.recordset")
	sql="select a.id,a.title,a.title_en,a.px,a.lm,b.title_lm,b.title_lm_en from tol_co a,tol_lm b where a.lm=b.id_lm "&sq&" order by a.px desc,a.id desc"
	rs.open sql,conn,1,1
	if not rs.eof then
		pagenum=25
		rs.pagesize=pagenum
   ...
   while not rs.eof and a<=pagenum
    ...
    <tr class=tdbg  onMouseOver="DoHL();" onMouseOut="DoLL();" onClick="DoSL();">
     <td align="center"><%=rs("id")%></td>
      ...
      <td ><a href="default.asp?id_lm=<%=rs("lm")%>"><strong>[</strong><%=rs("title_lm")%> <%=rs("title_lm_en")%><strong>]</strong></a> <a href="edit.asp?id=<%=rs("id")%>"><%=rs("title")%>  <%=rs("title_en")%></a></td>
      <TD align='center'><input name="<%=rs("id")%>" id="<%=rs("id")%>" type="text" value="<%=rs("px")%>" size="5" maxlength="10" disabled></TD>
      <td align="center" ><a href="edit.asp?id=<%=rs("id")%>">修改</a> | <a style="11display:none;" href="make.asp?id=<%=rs("id")%>&ac=del"  onClick="return confirm('确定要删除该数据吗?')">删除</a></td>
    </tr>
	<%
	a=a+1
	rs.movenext
                ...
                <TD height=30 align="left">				&nbsp;&nbsp; &nbsp;&nbsp;
                  ...
                  <input name=chkOthers onClick=CheckOthers(this.form,"yes")  type=button value='反选'>
                    ...
                    <option value="px">修改选中排序</option>
                     <option value="del" style="display:none;">删除选中信息</option>
                    </select>
