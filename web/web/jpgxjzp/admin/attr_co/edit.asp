...
'读取数据
sql="select * from attr_co where id="&id&""
...
set rs=server.CreateObject("adodb.recordset")
sql="select add_xx from attr_lm where id_lm="&lm&""
rs.open sql,conn,1,1
if rs.eof then
...
set rs=server.CreateObject("adodb.recordset")
sql="select * from attr_lm  order by px desc,id_lm desc"
rs.open sql,conn
if not rs.eof  then
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
    ...
    <tr class="tdbg" style="display:none;"> 
	   ...
	   <select name="lm" onchange="check_display()">
		...
		sql="select id_lm,title_lm,add_xx,xia from attr_lm where fid=0 order by px desc,id_lm desc"
		...
		sub addinfo(fid,i)
			i=i&"&nbsp;"
			sql="select id_lm,title_lm,add_xx,xia from attr_lm where fid="&fid&" order by px desc,id_lm desc"
    ...
    <tr class="tdbg" >           
     <td width="120" align="right"><strong>图片信息标题：</strong></td>          
      <td ><div  1style="display:none;"><input name="title" type="text" id="title"  size="44" maxlength="150" value="<%=title%>">              
    ...
    <tr class="tdbg" id="dis_img_sl"  <%if dis_img_sl="no" then response.write"style='display:none;'" end if%> style="display:none;">          
