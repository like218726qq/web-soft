...
sql="select title from pro_co  where id="&pro_id&""
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="border">
    ...
    <tr class="tdbg">
        ...
        <td><a href="default.asp">信息管理首页</a>&nbsp;|&nbsp;<a href="add.asp?pro_id=<%=pro_id%>">添加信息</a></td>
    </tr>
</table>
<BR>
...
<TABLE class=border cellSpacing=1 cellPadding=2 width="100%" border=0>
    <TBODY>
      <TR class=title >
        ...
        <%
        set rs=server.CreateObject("adodb.recordset")
        sql="select a.pro_id,a.id,a.lm,a.title,a.color_font,a.img_sl,a.wtime,a.tuijian,a.hot,a.pass,a.read_num,a.px,b.title_lm from attr_co a,attr_lm b where a.lm=b.id_lm "&sq&" order by a.px desc,a.id desc"
		'response.Write sql 
		'response.End()
        rs.open sql,conn,1,1
        if not rs.eof then
    ...
    <TR class=tdbg onMouseOver="DoHL();" onMouseOut="DoLL();" onClick="DoSL();">
        ...
        <TD>
        <strong><%=pro_title%>[</strong><A href="default.asp?id_lm=<%=rs("lm")%>"><%=rs("title_lm")%></A><strong>]</strong><a href="edit.asp?id=<%=rs("id")%>"><font color="<%=rs("color_font")%>"><%=rs("title")%></font></a>
        <%
        if rs("img_sl")<>"" then 
        ...
        <TD align=center> 
        <A href="edit.asp?id=<%=rs("id")%>&pro_id=<%=pro_id%>" >修改</A>|
     ...
     <!--
        ...
        <!--删除-->
        <A onClick="return confirm('提示:真的要删除此信息吗？');" href="make.asp?ac=del&id=<%=rs("id")%>" >删除</A>
        </TD>
     </TR>
<%a=a+1
rs.movenext
      ...
      <TBODY><TR>
        <TD height=30 >
         ...
         &nbsp; 
          ...
          <option value="tuijian2">推荐选中信息</option>
