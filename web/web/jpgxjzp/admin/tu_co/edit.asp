...
'读取数据
sql="select * from tu_co where id="&id&""
...
'获取该图片信息所在栏目是否可添加图片信息
set rs=server.CreateObject("adodb.recordset")
sql="select add_xx from img_lm where id_lm="&lm&""
rs.open sql,conn,1,1
if rs.eof then
	call msg("该图片信息对应的栏目不存在或已删除","")
else
	add_xx=rs("add_xx")
end if
rs.close
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="border">  
		<tr class="topbg"> 
			<td  colspan="2"  align="center">图片信息中心管理----修改图片信息</td>  
		</tr>  
		<tr class="tdbg">   
			<td width="70" height="26" ><strong>管理导航：</strong></td>
			<td><a href="default.asp">图片信息管理首页</a>&nbsp;|&nbsp;<a href="add.asp">添加图片信息</a></td>
		</tr>
	</table>
<br />
<form method="POST" name="form1"  onsubmit="return Check()" action="editt.asp" >
<input name="id" value="<%=id%>" type="hidden"> 
<input name="pro_id" value="<%=pro_id%>" type="hidden"> 
<input  type="hidden" name="w_url" value="<%=w_url%>">
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
    <tr class="title">      
         <td  align="center" colspan="2"><b>修改图片信息</b></td>   
    </tr>    
    <tr class="tdbg" style="display:none;"> 
	   ...
	   <select name="lm" onchange="check_display()">
		...
		sql="select id_lm,title_lm,add_xx,xia from img_lm where fid=0 order by px desc,id_lm desc"
		...
		sub addinfo(fid,i)
			i=i&"&nbsp;"
			sql="select id_lm,title_lm,add_xx,xia from img_lm where fid="&fid&" order by px desc,id_lm desc"
    ...
    <tr class="tdbg" >           
     <td width="120" align="right"><strong>图片信息标题：</strong></td>          
      <td ><div  1style="display:none;"><input name="title0" type="text" id="title0"  size="44" maxlength="150" value="<%=title%>" disabled="disabled">
      <input name="title" type="hidden" id="title"  size="44" maxlength="150" value="<%=title%>" />               
			 ...
			 <script>document.form1.color_font.value="<%=color_font%>"</script></div>
       </td >
    </tr>
    
    <tr  class="tdbg" id="dis_link_url" <%if dis_link_url="no" then response.write"style='display:none;'" end if%>  style="display:none;">
    <td align="right"><strong>连接地址：</strong></td>
    <td><input name="link_url" type="text" id="link_url" size="50" maxlength="150" value="<%=link_url%>" ></td>
    </tr>
    <tr class="tdbg" id="dis_info_from" <%if dis_info_from="no"  then response.write"style='display:none;'" end if%>  style="display:none;">           
         <td width="120" align="right"><strong>图片信息来源：</strong></td>          
         <td ><input name="info_from" type="text" id="info_from"   maxlength="50"  value="<%=info_from%>">
          &nbsp; 图片信息作者：<input name="info_author" type="text" id="info_author"   maxlength="50" value="<%=info_author%>"/></td >
    </tr>
    <tr class="tdbg"  id="dis_f_body"  <%if dis_f_body="no" then response.write"style='display:none;'" end if%>  style="display:none;">
        <td align="right" valign="top"><strong><br>图片信息精要：</strong></td>
        <td ><textarea name="f_body" cols="72" rows="4" id="f_body"><%=f_body%></textarea></td>
    </tr>
    <tr class="tdbg" id="dis_z_body"  <%if dis_z_body="no"  then response.write"style='display:none;'" end if%>  style="display:none;">          
        <td width="120" align="right" valign="middle"><p><strong>图片信息内容：<br></strong><br></td>
    <td >
    ...
    <tr class="tdbg" id="dis_dow_sl"  <%if dis_dow_sl="no" then response.write"style='display:none;'" end if%>  style="display:none;">          
    ...
    <tr class="tdbg" id="dis_vid_sl"  <%if dis_vid_sl="no" then response.write"style='display:none;'" end if%>  style="display:none;">          
