...
<%
	...
	select case ac
		   case"del"
				...
				sql="select * from info_lm where id_lm in("&id&")"
				...
				if not rs.eof then 
					...
					while not rsb.eof
						...
						sql="select * from pro_img where info_id="&rsb("id")&""
			...
			case"px"
				...
				for a=0 to ubound(shu)
					conn.execute"update info_lm set px="&request(shu(a))&" where id_lm="&shu(a)&" "
				next
			end select
		...
		while not rsa.eof
			...
			'删除属于该栏目的信息和该信息的图片
				...
				while not rsk.eof
					...
					sql="select * from pro_img where info_id="&rsk("id")&""
