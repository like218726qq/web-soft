...
.time{ border-bottom:1px dotted #dcdcdc; width:100%; height:28px; line-height:28px;}
 ...
 <%
		...
		if lm<>"" and isnumeric(lm) then
		  ...
		  sql="select * from info_co where pass='yes' and (lm in(select id_lm from info_lm where fid=13 ) or lm=13) order by px desc,id desc"
		end if
		'response.Write sql
...
<tr>
    ...
    <div class="title">
    <a href=news_show.asp?id=<%=rs("id")%>&ntype=<%=rs("lm")%> ><%=rs("title")%>
    </a>
    </div></td>
    <td align="right">
    <a href=news_show.asp?id=<%=rs("id")%>&ntype=<%=rs("lm")%>>
