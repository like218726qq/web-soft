...
<table border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top:15px; width:80%;">
 ...
 <%
		...
		pra=""
		sq=" and (lm in( 52 ) or lm in( select id_lm from info_lm where fid=52 )) "
		end if
		if keyword<>"" then 
	...
	%>
   <tr> <td width="200"><table border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td width="200"><a href="<%=rs("link_url")%>" target="_blank"><img src="../<%=rs("img_sl")%>" width="200" height="200" /></a></td>
                </tr>
                <tr>
                  <td align="center"><a href="<%=rs("link_url")%>" target="_blank"><%=rs("title")%></a></td>
                </tr>
              </table></td>
              <td><%=rs("f_body")%></td>
              </tr>
	...
	%>
    <tr><td><div style=" height:30px; line-height:30px; width:80%;white-space:nowrap;">暂无相关信息</div></td></tr>
	<%
		end if	
		rs.close
		set rs=nothing
