...
If oUpload.Err > 0 Then
	...
	Case 1
		call msg("请选择有效的上传文件","location='upd.asp?frameid="&frameid&"&kuang="&kuang&"'")
	Case 2
		call msg("你上传的文件总大小超出了最大限制（" & maxsize & "KB）","location='upd.asp?frameid="&frameid&"&kuang="&kuang&"'")
	End Select
End If
...
response.write"<script>location='updloadd.asp?frameid="&frameid&"&kuang="&kuang&"&img_sl="&path&"/"&sSaveFileName&"'</script>"
response.End()
...
Sub CheckValidExt(sExt)
	...
	Next
	If b = False Then
		call msg("\n\n请选择一个有效的文件，\n支持的格式有（"+AllowExt+"）","location='upd.asp?frameid="&frameid&"&kuang="&kuang&"'")
		Response.End
	End If
End Sub
%>