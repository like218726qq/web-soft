...
<div id="MAIN">
    ...
    <div id="con_main">
        <div id="left">
            <div id="co">
                <div class="co_title"><img src="images/zp_03.jpg" /></div>
                <div class="co_list">
                    <ul>
                    <%
	set rs=server.CreateObject("adodb.recordset")
	sql="select id,title_en from tol_co where lm=4 order by px desc,id desc"
	rs.open sql,conn,1,1
	while not rs.eof
%>
                        <li><img src="images/index_34.jpg" /><a href="jobs.asp?id=<%=rs("id")%>"><%=rs("title_en")%></a></li>
<%
    rs.movenext
	wend
	rs.close
...
%>
        ...
        <div id="right">
            <div id="right_pr_title">
                <h2><%=title%></h2>
                <span>Your location：Home > Recruitment > <%=title%></span>
                <div class="clear"></div>
            </div>
            <div id="right_pr_co">
            <%=z_body%>
