...
<%
	...
	sql="select title_lm_en from info_lm where id_lm="&lm&" "
	rsa.open sql,conn,1,1
	if not rsa.eof then
		dao=" &gt; "&rsa("title_lm_en")
		title_lm=rsa("title_lm_en")
	end if
	rsa.close
	set rsa=nothing
...
%>
    ...
    <div style="text-align:right; line-height:22px; color:#36648c; padding:10px;">Time：<%=wtime%></div>
        <%
    if img_sl<>"" then
    %>
    ...
    %>
        <div style="width:95%; margin:0 auto; line-height:200%;"><%=z_body%></div>
             <div class="clear"></div>
       <div class="page"><a href="javascript:history.back();">[Back]</a></div>
            </div>
            <div class="clear"></div>
        </div>
