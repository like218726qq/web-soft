...
<div id="MAIN">
	...
	sql="select top 8 id,title_en from tol_co where lm=2 order by px desc,id desc"
	rs.open sql,conn,1,1
	while not rs.eof
%>
                    	<li><img src="images/index_34.jpg" /><a href="service.asp?id=<%=rs("id")%>"><%=rs("title_en")%></a></li>
<%
    rs.movenext
	wend
	rs.close
...
<div class="int_co_z"><% sql="select top 1 * from tol_co where lm=6 and id=1 order by px desc,id desc"
	...
	rs.close
	set rs=nothing
	%><A href="about.asp">[Learn more]</A></div>
            ...
            <div id="right_mian">
              ...
              <div class="news_list">
                        <ul>
                         <%
sql="select top 8 id,title_en,wtime,img_sl from info_co where pass='yes' and lm in(2,3) order by px desc,id desc"
set rs=server.CreateObject("adodb.recordset")
rs.open sql,conn,1,1
	if not rs.eof then
	do while not rs.eof 
	   %>
                                	<li><img src="images/d.jpg" /><a href="news_show.asp?id=<%=rs("id")%>"><%=left(rs("title_en"),20)%></a><span>[<%=formatdate(rs("wtime"))%>]</span></li>
<%rs.movenext     
	  loop
	end if
	rs.close
	set rs=nothing%>
                ...
                <div id="cx">
                    ...
                    <div class="cx_co">
                        <ul>
                         <%
sql="select top 6 id,title_en,link_url,img_sl from info_co where pass='yes' and lm in(5) order by px desc,id desc"
set rs=server.CreateObject("adodb.recordset")
rs.open sql,conn,1,1
	if not rs.eof then
	do while not rs.eof 
	   %>
                            <li><A href="<%=rs("link_url")%>" target="_blank"><img src="../<%=rs("img_sl")%>" width="120" height="39" /></A><A href="<%=rs("link_url")%>" target="_blank"><%=rs("title_en")%></A></li>
 <%rs.movenext     
	  loop
	end if
	rs.close
