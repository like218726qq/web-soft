...
<div id="MAIN">
	...
	sql="select id,title_en from tol_co where lm=3 order by px desc,id desc"
	rs.open sql,conn,1,1
	while not rs.eof
%>
                        <li><img src="images/index_34.jpg" /><a href="price.asp?id=<%=rs("id")%>"><%=rs("title_en")%></a></li>
<%
    rs.movenext
	wend
	rs.close
...
%>
        ...
        <div id="right">
            <div id="right_pr_title">
                ...
                <span>Your location：Home >  Cargo tracking</span>
                <div class="clear"></div>
            </div>
            <div id="right_pr_co">
              <div class="Links">
    <%
sql="select id,title_en,img_sl,link_url from info_co where pass='yes' and lm in(5) order by px desc,id desc"
set rs=server.CreateObject("adodb.recordset")
rs.open sql,conn,1,1
if not rs.eof then
		while not rs.eof 
%>
             	<a href="<%=rs("link_url")%>" target="_blank"><img src="../<%=rs("img_sl")%>" alt="<%=rs("title_en")%>" width="135" height="40" /><br /><%=rs("title_en")%></a>
<%
