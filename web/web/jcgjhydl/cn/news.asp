...
<%
	...
	if lm<>"" then
		...
		sql="select top 1 id_lm,title_lm from info_lm where fid=1 order by px desc,id_lm desc"
		set rs=server.CreateObject("adodb.recordset")
		rs.open sql,conn,1,1
		if not rs.eof then
			lm=rs("id_lm")
...
<%
	set rs=server.CreateObject("adodb.recordset")
	sql="select id,title,color_font,wtime from info_co where pass='yes' "&sq&" order by px desc,wtime desc,id desc"
	rs.open sql,conn,1,1
	if not rs.eof then
		pagenum=12
		rs.pagesize=pagenum
...
%>
        ...
        <li><span class="fr"><%=formatdate(rs("wtime"))%></span><a href="news_show.asp?id=<%=rs("id")%>"><font style="color:<%=rs("color_font")%>;"><%=rs("title")%></font></a></li>
...
%></ul>
             <div class="clear"></div>
       <div class="page"><%if pagenum<>"" then call listpage_num(pra) end if%></div>
            </div>
            <div class="clear"></div>
        </div>
