        ...
        <div id="left">
            <div id="co">
                ...
                <div class="co_list">
                    ...
                    <%
	set rs=server.CreateObject("adodb.recordset")
	sql="select id,title from tol_co where lm=4 order by px desc,id desc"
	rs.open sql,conn,1,1
	while not rs.eof
%>
                        <li><img src="images/index_34.jpg" /><a href="jobs.asp?id=<%=rs("id")%>"><%=rs("title")%></a></li>
<%
    rs.movenext
	wend
	rs.close
...
%>
        ...
        <div id="right">
            <div id="right_pr_title">
                <h2><%=title%></h2>
                <span>您现在所在位置：首页 > 人才招聘 > <%=title%></span>
                <div class="clear"></div>
            </div>
            <div id="right_pr_co">
            <%=z_body%>
