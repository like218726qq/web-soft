...
<body>
	...
	<div class="main">
    	...
    	<div class="content">
        	<div class="cont1">
            	<h4>视频播放<b>Vedio Play</b></h4>
                <div class="cont1con">
                    <p>
					<%
                        set rs=server.CreateObject("adodb.recordset")
                        sql="select top 1 id,link_url from info_co where id=22"
                        rs.open sql,conn,1,1
                        while not rs.eof
                    %>                    
						<embed src="<%=rs("link_url")%>" allowFullScreen="true" quality="high" width="238" height="188" align="middle" allowScriptAccess="always" type="application/x-shockwave-flash"></embed>
					<%
                        rs.movenext
                        wend
                        rs.close
...
%>
        	...
        	<h4>
                ...
                <ul class="aaa">
				   ...
				   <%
                        ...
                        while not rsa.eof
                    %>
                	<li><a <%if a=1 then%> class="ato" <%end if%> style="cursor:pointer" id="pronav<%=a%>"><%=rsa("title_lm")%></a></li>
                    <%
						a=a+1
						rsa.movenext
						wend
    ...
    <div id="pic_box">
        ...
        <%
			set rs=server.CreateObject("adodb.recordset")
			sql="select top 8 id,title,img_sl from pro_co where lm in (select id_lm from pro_lm where fid=12) and tuijian='yes' and pass='yes'  order by px desc,id desc"
			rs.open sql,conn,1,1
			while not rs.eof
		%>
        <div class="pic_item  fl5">
             <table border="0" align="center" cellpadding="0" cellspacing="0">
              <tr>
                <td width="204" height="120" align="center" ><a href="dispro.asp?id=<%=rs("id")%>"><img src="<%=rs("img_sl")%>" border="0" width="180" height="120" style="border:3px solid #5b9600;"/></a></td>
              </tr>
