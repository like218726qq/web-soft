...
<table border="0" cellpadding="0" cellspacing="0">
	 ...
	 <table border="0" cellpadding="0" align="left" cellspacing="0">
		...
		<%
			...
			while not rsa.eof
		%>
		<tr><td style=" width:200px; height:25px; text-indent:1em; background:url(images/leftbg.png) left top no-repeat;"><a href="categry_view.asp?dlm=<%=rsa("id_lm")%>"><%=rsa("title_lm_en")%></a></td></tr>
        ...
        sql="select id_lm,title_lm,title_lm_en from pro_lm where fid="&rsa("id_lm")&" order by px desc,id_lm desc"
	...
	%>
		<%
			while not rsb.eof
		%>
            <li><a href="product.asp?dlm=<%=rsa("id_lm")%>&id_lm=<%=rsb("id_lm")%>" class="p1"><%=rsb("title_lm_en")%></a></li>
            <%
			rsb.movenext
			wend
		%>
