...
<%
	...
	if lm<>"" then
		...
		sql="select top 1 id_lm,title_lm from info_lm where fid=16 order by px desc,id_lm desc"
		set rs=server.CreateObject("adodb.recordset")
		rs.open sql,conn,1,1
		if not rs.eof then
			lm=rs("id_lm")
...
%>
    ...
    <div class="pro-right-bar">
   	  <div class="proT"><p>News <%=dao%></p></div>
      <div class="proM">
       <table border="0" cellpadding="0" cellspacing="0">
      <tr>
...
%>
    <table border="0" cellpadding="0" align="center" cellspacing="0">
	<tr><td style=" width:110px; height:90px;"><a href="news_show.asp?id=<%=rs("id")%>"><img src="../<%=rs("img_sl")%>" alt="" /></a></td>
	...
	<%=cutStr(rs("z_body"),90)%>......<a href="news_show.asp?id=<%=rs("id")%>">read more</a>
	</td></tr>
	</table>
