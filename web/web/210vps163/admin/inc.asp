...
<%  Session.CodePage=65001
	...
	sub openconn()
		...
		connstr="DBQ="+server.mappath("../root.mdb")+";DefaultDir=;DRIVER={Microsoft Access Driver (*.mdb)};"
		conn.open connstr 
	end sub
	...
	sub opconn()
		set conn=server.createobject("adodb.connection")
		connstr="DBQ="+server.mappath("../admin/root.mdb")+";DefaultDir=;DRIVER={Microsoft Access Driver (*.mdb)};"
		conn.open connstr 
	end sub
...
End Function
	...
	sub listpage(key)
		...
		listp=listp&"[<a href='?page=1"&key&"'  style=' color:#4881c6;'>首页</a>][<a href='?page="&pageno-1&key&"' style=' color:#4881c6;'>上一页</a>]&nbsp;"       
		end if
		if cint(pageno)=cint(pagezong) then
		listp=listp&"[下一页][尾页]"
		else
		listp=listp&"[<a href='?page="&pageno+1&key&"'  style='color:#4881c6;'>下一页</a>][<a href='?page="&pagezong&key&"'  style=' color:#4881c6;'>尾页</a>]"        
		end if 
	...
	sub listpageen(key)
		listp=listp&"<TABLE cellSpacing=0 cellPadding=0  align=center border=0 style='color:#666666' ><TR>"&chr(10)
		listp=listp&"<TR>"&chr(10)
		listp=listp&"<TD >the<font color='#ff0000'>"&pageno&"</font>page|A total of<font color='#ff0000'>"&pagezong&"</font>|<font color='#ff0000'>"&pagenum&"</font>item/page|<font color='#ff0000'>"&counter&"</font>item</td>"&chr(10)
		listp=listp&"<td width='100'>&nbsp;</td>"&chr(10)
		listp=listp&"<td>"
		if pageno=1 then
	...
	'提取字段
	sub getfield(table,fiels,id,longs)
		set rs = server.CreateObject("adodb.recordset")
		sql = "select " & fiels & " from " & table & " where id=" & id
		'if wherestr<>"" then sql = sql & " where " & wherestr
		rs.open sql,conn,1,1
		str = rs(fiels)
		longs=int(longs)
		if longs<>0 then str = getstr(str,longs)
		response.Write(str)
		rs.close
		set rs = nothing
	end sub
%>
<%'call getfield("tol_co","z_body",3)%>
